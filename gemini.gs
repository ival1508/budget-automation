...
const GEMINI_MODEL_ID = 'gemini-3.6-flash';      // primary - reliable
const BACKUP_MODEL_1  = 'gemini-3.5-flash-lite'; // fastest tier
const BACKUP_MODEL_2  = 'gemini-3.7-flash';      // last - currently congested

/**
 * Invokes the Gemini API with automatic model fallback for HTTP 503 / 429 capacity spikes.
 * Capped at 1 attempt per model for fast failover.
 * 
...
function callGeminiApiWithRetry(payload, apiKey, preferredModel) {
  ...
  for (let m = 0; m < modelsToTry.length; m++) {
    ...
    Logger.log(`Gemini API (${modelId}) returned error ${responseCode}: ${responseText}`);
