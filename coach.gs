...
- Accuracy: NEVER invent or hallucinate financial amounts not present in the JSON payload.`;
  ...
  Logger.log(`Generating Coach Brief via Gemini (${typeof GEMINI_MODEL_ID !== 'undefined' ? GEMINI_MODEL_ID : 'gemini-3.5-flash-lite'})...`);
  try {
    const responseText = callGeminiApiWithRetry(apiPayload, apiKey, typeof GEMINI_MODEL_ID !== 'undefined' ? GEMINI_MODEL_ID : 'gemini-3.5-flash-lite');
...
Keep the tone concise, encouraging, and clear.`;
  ...
  const modelId = typeof GEMINI_MODEL_ID !== 'undefined' ? GEMINI_MODEL_ID : 'gemini-3.5-flash-lite';
